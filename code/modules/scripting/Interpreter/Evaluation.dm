...
/n_Interpreter/proc/Eval(node/expression/exp)
	...
	else if(istype(exp, /node/expression/oprand))
...
/n_Interpreter/proc/EvalOperator(node/expression/oprand/exp)
	if(istype(exp, /node/expression/oprand/binary))
		var/node/expression/oprand/binary/bin = exp
		switch(bin.type)
			if(/node/expression/oprand/binary/Equal)
				return Equal(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/NotEqual)
				return NotEqual(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Greater)
				return Greater(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Less)
				return Less(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/GreaterOrEqual)
				return GreaterOrEqual(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/LessOrEqual)
				return LessOrEqual(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/LogicalAnd)
				return LogicalAnd(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/LogicalOr)
				return LogicalOr(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/LogicalXor)
				return LogicalXor(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/BitwiseAnd)
				return BitwiseAnd(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/BitwiseOr)
				return BitwiseOr(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/BitwiseXor)
				return BitwiseXor(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Add)
				return Add(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Subtract)
				return Subtract(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Multiply)
				return Multiply(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Divide)
				return Divide(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Power)
				return Power(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/oprand/binary/Modulo)
				return Modulo(Eval(bin.exp), Eval(bin.exp2))
			else
				RaiseError(new/runtimeError/UnknownInstruction())
	else
		switch(exp.type)
			if(/node/expression/oprand/unary/Minus)
				return Minus(Eval(exp.exp))
			if(/node/expression/oprand/unary/LogicalNot)
				return LogicalNot(Eval(exp.exp))
			if(/node/expression/oprand/unary/BitwiseNot)
				return BitwiseNot(Eval(exp.exp))
			if(/node/expression/oprand/unary/group)
				return Eval(exp.exp)
			else
				RaiseError(new/runtimeError/UnknownInstruction())
